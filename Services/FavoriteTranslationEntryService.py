...
class FavoriteTranslationEntryService:
    ...
    def delete_translated_entry(self, user_id: str, entry_id: str):
        ...
        db_entry = self.favorite_translation_entry_repository.get_translated_entry_by_id(entry_id)

        if db_entry and db_entry["user_id"] == user_id:
            self.favorite_translation_entry_repository.delete_translated_entry(entry_id)
        else:
